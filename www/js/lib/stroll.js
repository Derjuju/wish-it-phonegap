...
(function(){
	...
	var THRESHOLD = window.innerWidth/4; //80;
	...
	function TouchList( element ) {
		...
		this.indentX = element.parentNode.offsetLeft;
	...
	TouchList.prototype.onTouchStart = function( event ) {
		...
		if( event.touches.length === 1 ) {
			...
			touchStartX = event.touches[0].clientX - this.indentX;
	...
	TouchList.prototype.onTouchMove = function( event ) {
		if( event.touches.length === 1 ) {
			...
			touchMoveX = event.touches[0].clientX - this.indentX;
			swipeMethod = null;
				...
				/*else if( touchMoveY < touchStartY - THRESHOLD ) {
			...
			if(swipeMethod != null)
			{
				//alert("swipe in stroll : "+swipeMethod + "["+(touchMoveX-touchStartX)+"]\n["+touchStartX+"-"+THRESHOLD+"="+(touchStartX - THRESHOLD)+"]["+touchMoveX+"]\n["+touchStartX+"+"+THRESHOLD+"="+(touchStartX + THRESHOLD)+"]["+touchMoveX+"]");
				var event = document.createEvent( "HTMLEvents", 1, 2 );
				event.initEvent(swipeMethod , true, true );
	...
	TouchList.prototype.onTouchEnd = function( event ) {
		...
		if( Math.abs( this.velocity ) > 4 || Math.abs( distanceMoved ) > 10 || swipeMethod != null) {
			event.preventDefault();
		}
	};
